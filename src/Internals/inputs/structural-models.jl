...
function niche_model(S::Int, C::AbstractFloat)
    ...
    C >= 0.5 && throw(
        ArgumentError(
            "The connectance cannot be larger than 0.5." * "Given value of C=$C.",
        ),
    )
    # Build niche matrix.
    A = zeros(Bool, S, S)
    ...
    range_list[argmin(body_size_list)] = 0.0 # Smallest species has no range.
